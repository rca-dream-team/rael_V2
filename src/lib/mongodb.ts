...
export async function getComments(postId: string, currentUserId?: string): Promise<IComment[]> {
  if (!postId) {
    return [];
  }
  ...
  try {
    ...
    const commentIds: string[] = comments
      .map((c: any) => (c._id || c.id)?.toString())
      .filter(Boolean);
    ...
    if (commentIds.length > 0) {
      likesCounts = await db.collection('comment_likes').aggregate([
        { $match: { commentId: { $in: commentIds } } },
        { $group: { _id: '$commentId', count: { $sum: 1 } } }
      ]).toArray();

      if (currentUserId) {
        userLikes = await db.collection('comment_likes').find({
    ...
    const likesMap = new Map<string, number>(
      likesCounts.map((item: any) => [item._id.toString(), item.count])
    );
    const userLikedSet = new Set<string>(
      userLikes.map((item: any) => item.commentId.toString())
    );
...
export async function deleteComment(id: string) {
  ...
  try {
    ...
    const result = await db.collection<any>('comments').deleteOne({
      $or: [
        { _id: id },
        { id: id }
      ]
